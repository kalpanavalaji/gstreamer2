...
typedef struct play
{
	GstElement *pipeline, *source,*decodebinn, *Vsink,*Are_sa,*Asink,*Aconv1,*Vconv,*Aconv,*adderr;
	GstState state;
	GMainLoop *loop;
	GstElement *bin,*mixedqueue ,*audioqueue,*videoqueue;
	GstPad *audio_queue_src_pad,*adder_queue_sink_pad;
...
gint main(int argc ,char *argv[])
{
	...
	data.pipeline = gst_pipeline_new ("adder-decodebin");
...
data.adderr    = gst_element_factory_make ("adder", "data.Aadderr");
	...
	bus = gst_element_get_bus (data.pipeline);
	 ...
	 gst_bin_add_many (GST_BIN                                                                                                 (data.pipeline),data.source,data.decodebinn,data.videoqueue,data.Vconv,data.Vsink,data.audioqueue, data.adderr,data.Aconv,data.Asink,NULL);//add ele in bin
